...
class LimitFigure:
    ...
    def add_experiment(self, name):
        ...
        if name=='grand_10k':
            ...
            if self.e_power==1:
                self.ax.annotate('ANITA I - IV',
                                 xy=(1.5e9, 1e-14*self.e_bins), xycoords='data',
            ...
            if self.e_power==1:
                self.ax.annotate('ARA (2x1yr)',
                                 xy=(2.5e8, 1.6e-15*self.e_bins), xycoords='data',
                                 horizontalalignment='left', color='#2288AA', rotation=-40, fontsize=12)
                # self.ax.annotate('ARA (2x4yr)',
    ...
    def build_base_plot(self, group='clean', experiments=None, models=None):
        if group=='all':
            ...
            if experiments is None:
                experiments = ['anitaiv', 'auger', 'ara', 'arianna',
                               'ice_cube_ehe', 'ice_cube_hese_data', 'ice_cube_mu_fit']
            if models is None:
                models = ['kotera', 'ahlers']
        elif group=='ara_src':
