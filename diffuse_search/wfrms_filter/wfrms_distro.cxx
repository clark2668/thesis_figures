...
////	make plots of WFRMS distributions for thesis
////////////////////////////////////////////////////////////////////////////////
...
int main(int argc, char **argv)
{
	...
	wfrms_plots_rf[0] = new TH2D("VPol Data","VPol Data",200,-5,5,30,0,30);
	wfrms_plots_rf[1] = new TH2D("HPol Data","HPol Data",200,-5,5,30,0,30);
	...
	for(int i=0; i<2; i++){
		...
		passed_events[i] = new TH1D("","",25,0,25);
		eff[i] = new TH1D("","",25,0,25);
	}
	...
	for(int file_num=6; file_num<argc; file_num++){
		...
		for(int event=0; event<numEntries; event++){
			...
			if(SNRs[0]>25.) SNRs[0]=24.;
			if(SNRs[1]>25.) SNRs[1]=24.;
	...
	*/
	...
	for(int pol=0; pol<2; pol++){
		...
		projections_rf[pol]->GetXaxis()->SetTitle("#xi = log10(cos(#theta))");
		projections_rf[pol]->GetYaxis()->SetRangeUser(0.1,1e7);
		if(isSim){
			projections_rf[pol]->GetYaxis()->SetRangeUser(0.1,1e4);
			if(pol==0) projections_rf[pol]->SetTitle("VPol Sim");
			if(pol==1) projections_rf[pol]->SetTitle("HPol Sim");
		}
		projections_rf[pol]->GetYaxis()->SetTitleOffset(1.3);
		projections_rf[pol]->GetYaxis()->SetTitleSize(0.045);
		projections_rf[pol]->GetYaxis()->SetLabelSize(0.045);
		projections_rf[pol]->GetXaxis()->SetTitleSize(0.045);
		projections_rf[pol]->GetXaxis()->SetLabelSize(0.045);
		// projections_rf[pol]->GetYaxis()->SetLabelOffset(1.3);
		...
		// 	TLegend *leg2 = new TLegend(0.5,0.9,0.9,0.7);
		// 	leg2->AddEntry(projections_rf[pol],"RF Data","l");
		// 	leg2->AddEntry(projections_cal[pol],"Tagged Cal Pulsers","l");
		// 	leg2->Draw();
		// }
		gPad->SetLogy();
	}
	char title[500];
	if(isSim)
		sprintf(title, "/users/PAS0654/osu0673/GeomStuff/thesis_figures/diffuse_search/wfrms_filter/sim_A%d_c%d_%dEvents_Filter_RFEvents_Vpol%.1f_Hpol%.1f.png",station,config,int(num_total),0.1*double(thresholdBin_pol[0]) + 2.0,0.1*double(thresholdBin_pol[1])+2.0);
	else
		sprintf(title, "/users/PAS0654/osu0673/GeomStuff/thesis_figures/diffuse_search/wfrms_filter/data_A%d_c%d_%dEvents_Filter_RFEvents_Vpol%.1f_Hpol%.1f.png",station,config,int(num_total),0.1*double(thresholdBin_pol[0]) + 2.0,0.1*double(thresholdBin_pol[1])+2.0);
	c2->SaveAs(title);
	delete c2;
	for(int i=0; i<2; i++){
	...
	if(isSim){
		...
		for(int pol=0; pol<2; pol++){
			...
			c2->cd(pol+1);
				...
				eff[pol]->GetYaxis()->SetTitleOffset(1.3);
				eff[pol]->GetYaxis()->SetTitleSize(0.045);
				eff[pol]->GetYaxis()->SetLabelSize(0.045);
				eff[pol]->GetXaxis()->SetTitleSize(0.045);
				eff[pol]->GetXaxis()->SetLabelSize(0.045);		
		}
		sprintf(title,"/users/PAS0654/osu0673/GeomStuff/thesis_figures/diffuse_search/wfrms_filter/sim_A%d_c%d_%dEvents_Filter_Efficiency_Vpol%.1f_Hpol%.1f.png",station,config,int(num_total),0.1*double(thresholdBin_pol[0]) + 2.0,0.1*double(thresholdBin_pol[1])+2.0);
		c2->SaveAs(title);
	}
